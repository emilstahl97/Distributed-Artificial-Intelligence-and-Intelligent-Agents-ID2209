...
species Auctioneer skills:[fipa] {
	...
	reflex receive_propose_messages when: !empty(proposes) {
		...
		loop m over: proposes {
			...
			if (p >= lowestPrice) {
				// SOLD
				write("\t" + Guest(m.sender).name + " bought " + merch[m.contents[0]] + " for " + p + " euros.");
				sold <- true;
			...
			do end_conversation with: [ message :: m, contents :: [] ];
			prices <- prices + int(m.contents[1]);
			n <- n + 1;
		}
		// Compute new price
