/***
* Name: Assignment2
* Author: Emil Stahl & Peyman Peirovifar
...
global {
	...
	list<list<Initiator>> InitiatorList <- [];
...
species Initiator skills: [fipa] {
	...
	reflex readMessage when: (!(empty(proposes))) and self.auction_running{
		...
		loop a over: proposes {
			...
			list<unknown> x <- a.contents;
			
			bool is_buying <- bool(int(x[0]));
...
species Participant skills: [fipa, moving] {
	...
	reflex replyMessage when: (!empty(cfps)) {
		...
		list<unknown> c <- requestFromInitiator.contents;
		int x <- int(c[0]);
		int offer <- int(x);
